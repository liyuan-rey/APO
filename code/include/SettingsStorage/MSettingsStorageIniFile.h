...
public:
	...
	CSettingsStorage *CreateSubStorage(LPCTSTR SubName)
	{
		...
		_sntprintf_s(SubSectionName, MAX_PATH, _T("%s.%s"), m_SectionName, SubName);
	...
	{
		if (IniName) _tcsncpy_s(m_IniFileName, MAX_PATH, IniName, MAX_PATH);
		if (SectionName) _tcsncpy_s(m_SectionName, MAX_PATH, SectionName, MAX_PATH);
	}
...
public:
	...
	{	TCHAR Value[MAX_PATH]; \
		if (bSave) { \
			_sntprintf_s(Value, MAX_PATH, _T(token), Variable); \
			...
			return 1 == _sntscanf_s(Value, MAX_PATH, _T(token), &Variable); \
	}	}
	...
	inline bool SaveLoadItem(LPCTSTR szName, CColorRef& Variable, bool bSave)
	{
		...
		TCHAR token[] = _T("#%6X");
		if (bSave) {
			_sntprintf_s(Value, MAX_PATH, token, Variable.color);
			...
			DWORD dwClr;
			bool bRet = (1 == _sntscanf_s(Value, MAX_PATH, token, &dwClr));
	...
	inline bool SaveLoadItem(LPCTSTR szName, POINT& Variable, bool bSave)
	{
		...
		TCHAR token[] = _T("%d,%d");
		if (bSave) {
			_sntprintf_s(Value, MAX_PATH, token, Variable.x, Variable.y);
			...
			return 2 == _sntscanf_s(Value, MAX_PATH, token, &(Variable.x), &(Variable.y));
		}
	}
	...
	inline bool SaveLoadItem(LPCTSTR szName, SIZE& Variable, bool bSave)
	{
		...
		TCHAR token[] = _T("%d,%d");
		if (bSave) {
			_sntprintf_s(Value, MAX_PATH, token, Variable.cx, Variable.cy);
			...
			return 2 == _sntscanf_s(Value, MAX_PATH, token, &(Variable.cx), &(Variable.cy));
		}
	}
	...
	inline bool SaveLoadItem(LPCTSTR szName, RECT& Variable, bool bSave)
	{
		...
		if (bSave) {
			_sntprintf_s(Value, MAX_PATH, token, Variable.left, Variable.top, Variable.right, Variable.bottom);
			...
			return 4 == _sntscanf_s(Value, MAX_PATH, token, &(Variable.left), &(Variable.top), &(Variable.right), &(Variable.bottom));
		}
	}
	...
	inline bool SaveLoadItem(LPCTSTR szName, TCHAR* Variable, ULONG& Size, bool bSave)
	{
		...
		else
		{
			...
			_tcsncpy_s(Variable, Size, pTemp, Size);
			return true;
		}
	}
