...
protected:
	...
	virtual int OnItemCollection(UINT Index, CSettings &variable, bool bSave)
	{
		...
		TCHAR ItemName[MAX_ITEMNAME];
		_sntprintf_s(ItemName, MAX_ITEMNAME, _T("%i"), Index);
	...
	{
		_sntprintf_s(m_ItemName, MAX_ITEMNAME, _T("Item.%i"), Index);
		ItemName = m_ItemName;
		return 1;
	}
...
/*
		POINT pt = Variable;
		bool bRet = SaveLoadItem(szName, pt, bSave);
		if (!bSave)
			Variable = pt;

		return bRet;
*/
		POINT* pPoint = static_cast<POINT*>(&Variable);
		return SaveLoadItem(szName, *pPoint, bSave);
	}

	virtual bool SaveLoadItem(LPCTSTR szName, CSize& Variable, bool bSave)
	{
		SIZE* pSize = static_cast<SIZE*>(&Variable);
		return SaveLoadItem(szName, *pSize, bSave);
	}

	virtual bool SaveLoadItem(LPCTSTR szName, CRect& Variable, bool bSave)
	{
		RECT* pRect = Variable;
		return SaveLoadItem(szName, *pRect, bSave);
	}
